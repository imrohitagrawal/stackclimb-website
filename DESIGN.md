...
typography:
  scale:
    # The floor ON SCREEN. No HTML text on any page renders below 11px, gated by
    # tests/type-floor.spec.js. On PAPER the same floor holds since 2026-08-26 —
    # the Print-Floor Rule below, gated by tests/print-floor.spec.js (DEF-69).
    # FIVE keys used to sit here — caption-label 0.6,
...
**The Print-Floor Rule.** *(Added 2026-08-26, DEF-69 / D129.)* Under `@media print`, no text — `::before` and `::after` included — renders below 0.6875rem: 11px, which is 8.25pt on paper. It is the same number as the screen floor on purpose: one floor, one sentence. A revealed href prints **verbatim** — no uppercase, no tracking — because a URL in a different case is a different URL on a case-sensitive host, and paper is the one place a reader retypes it. Gated by `tests/print-floor.spec.js`, which emulates print and walks the pseudo-elements the screen gate cannot see; `cv.css`'s own 8.5pt reveal (11.33px) already cleared it, so `/cv` changed nothing.
