...
| P-7 | Surface AI-engineering practice: evals, monitoring, observability, roadmap | **PARTIAL 2026-08-15** — `/how-i-build` (package 5, D88) surfaces the CI-discipline and failure-driven practice from `docs/evidence/practice/` (golden-set gates, blocking-vs-advisory, the deploy-drift-watchdog artefact). What P-7 still names and this page does not cover: evals and observability specifically (SaafSaans/EvalAxis carry evaluation practice but it is not surfaced HERE, on the practice page) and a roadmap (deliberately absent sitewide — an undated roadmap is the class of claim `AGENTS.md` bars, per P-8's own resolution) | `/how-i-build` · D88 |
