...
| D31 | **The CV is a `/cv` page that prints cleanly, not a PDF upload** | Owner chose approach B. A PDF goes stale silently and nobody notices; a page is indexable, always current, and a recruiter can still print it to PDF and forward it. **Blocked on the owner for content:** title, years and one line of scope for Amazon, LimeRoad, Mobileum, Snapdeal and Subex. `PRODUCT.md:101` marks résumé content "Undecided — do not invent", and all that exists is a summary line | 08-09 |
