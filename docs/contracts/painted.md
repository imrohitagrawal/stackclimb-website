...
| 30 | `background-image` or `mix-blend-mode` anywhere up a text node's own chain, **on a node that has already passed its own font-size, visibility and fill-alpha checks** | **true, unconditionally past that point** | **Corrected 2026-09-01 (review round on #137): the row previously said "true, unconditionally" with no qualifier, which is FALSE — `inked()` checks `fontSize === 0`, `seen(n)`, then fill alpha `=== 0` BEFORE the ancestor loop that contains the abandon guard, so `color: transparent` over a background-image still correctly returns `false`, never reaching the guard.** The "abandon guard" is real, but it only fires for a node whose OWN paint already looks live — it stops the BACKDROP-COMPOSITING check, not the node's own visibility/alpha checks. It exists to avoid false-redding legitimate white-on-dark-gradient text (DEF-46's lesson). **This is a real, ever-widening exemption** on the backdrop-identity check specifically: one such property high in the tree silently drops colour-IDENTITY checking (row 24-25's `Δ=0` comparison) for the whole subtree under it, once a node has already cleared its own paint checks. Filed as DEF-78 rather than guarded here — declared as a known, accepted risk, not a contract gap, because removing it would reintroduce DEF-46 |
...
property name). **63 matrix rows total** (numbered 1-62, plus 43b — corrected 2026-09-01, review
round on #137: this section previously said 62, an off-by-one against the table's own row 43b),
plus the disposed-findings notes. This contract is not
